...
describe('Group3: Feature Flag 切替（Phase 2 Sub5 有効化 — Section 12準拠）', () => {
  beforeEach(() => {
    vi.resetAllMocks();
  });
...
describe('Group4: PDFUploadCard v2統合（Phase 2 Sub5 有効化）', () => {
  beforeEach(() => {
    vi.resetAllMocks();
    vi.mocked(v2Module.getUseMultiAgent).mockReturnValue(true);
  });
  ...
  it('extraction_failed → extractionFailed=true となり手入力誘導Alertが表示されること', async () => {
    ...
    fireEvent.click(screen.getByTestId('manual-input-from-alert'));
