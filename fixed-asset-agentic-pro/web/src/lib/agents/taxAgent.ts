 ...
 *
 * F-N06 Step 4: 前期末取得価額×10%判定（基通7-8-4(2)）
 * 現在は台帳未連携のため適用しない（Step4をスキップして UNCERTAIN または Step5 を適用）。
 * TODO: 台帳連携時に ExtractedLineItem.previousTermBookValue を追加し、Step4を有効化すること。
 *   参照: types/classify_pdf_v2.ts の TODO(F-N06 Step4) コメント
 */
...
| 4 | 支出額 < 前期末取得価額×10% | 【台帳未連携のため現在は適用しない: Step 4 はスキップしてStep 5へ進むこと】 | 基通7-8-4(2) | - |
| 5 | 資産の価値を高める or 耐用年数を延長することが明らか | CAPITAL（資本的支出） | 法人税法施行令第132条 | 5 |
| 6 | 上記いずれにも該当しない | UNCERTAIN | ユーザー判断 | 6 |

> **Step 4 に関する重要注記**: 前期末取得価額（基通7-8-4(2)）は固定資産台帳との連携が必要です。
> 現在は台帳データが未連携のため、Step 4 は適用しません。Step 4 の条件を満たす可能性がある場合でも
> Step 5 へ進み、情報不足なら UNCERTAIN で返してください。
> 台帳連携が実装された際には、入力データに previousTermBookValue フィールドが追加されます。
 ...
 *   ルールa: amount < thresholdAmount → EXPENSE（少額資産, 令第133条）
 *             ※ thresholdAmount はクライアントポリシーの閾値。デフォルト10万円。
 ...
 * @param thresholdAmount ポリシーの固定資産判定閾値（円）。省略時は 100,000 円（税法デフォルト）。
 * @returns autoResolved（確定判定済み） と needsLlm（LLM必要）の分離結果
 */
export function preScreenLineItems(
  items: ExtractedLineItem[],
  thresholdAmount: number = 100_000,
): PreScreenResult {
  ...
  for (const item of items) {
    ...
    // ルールa: thresholdAmount 未満 → EXPENSE確定（少額資産）
    // クライアントポリシーの閾値を使用。デフォルトは税法基準 10万円。
 ...
 * @param thresholdAmount クライアントポリシーの固定資産判定閾値（円）。省略時はデフォルト 100,000 円。
...
export async function runTaxAgent(
  ...
  // 前処理: ルールベースで確定できる明細を分離（ポリシー閾値を渡す）
  const { autoResolved, needsLlm } = preScreenLineItems(lineItems, thresholdAmount);
